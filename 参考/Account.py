...
class Market:
    def __init__(self,start,end):
        ...
        self.Mlogger.info("市场基本信息初始化完成")
        self.Mlogger.info("开始日期：" +datetime.datetime.strftime(self.today,"%Y-%m-%d"))
        self.Mlogger.info("结束日期：" +datetime.datetime.strftime(self.end,"%Y-%m-%d"))
    
        
        # 初始化数据库引擎
        
        self.db = WindData_sliceDB()
        self.Mlogger.info("数据信息初始化完成")
    ...
    def InquirePrice(self,code,dtype='CLOSE'):
        if dtype not in ['OPEN','HIGH','LOW','CLOSE']:
            self.Mlogger.info("查询" +code + "价格失败，输入参数为" + dtype)
            return 0
        try:
...
class Account(Market):
    def __init__(self,datestr,money=1000000):
        #初始化持仓记录 
        self.logger = logging.getLogger()
        self.logger.setLevel(0)
        fh = logging.FileHandler('Account持仓记录.txt', mode='w')
        formatter = logging.Formatter(' %(message)s')
        fh.setFormatter(formatter)
    ...
    #  更新每日的持仓信息
    def update(self,datestr,codes):
        self.__basic['日期'] = datestr
